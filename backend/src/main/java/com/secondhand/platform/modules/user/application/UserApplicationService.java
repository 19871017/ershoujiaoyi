...
public class UserApplicationService {
    ...
    public AccountSecurityResponse accountSecurity(Long userId) {
        ...
        List<AccountSecurityResponse> rows = jdbcTemplate.query("""
                ...
                securityScore(userId),
                loadRecentLoginDevices(userId)
        ), userId);
        if (rows.isEmpty()) {
        ...
        List<UserProfileResponse> rows = jdbcTemplate.query("""
                ...
                """, (rs, rowNum) -> {
                    ...
                    return new UserProfileResponse(
                            ...
                            rs.getString("gender"),
                            rs.getString("city"),
                            latestIpLocation(rs.getLong("id")),
                            rs.getString("bio"),
                            rs.getString("identity_status") == null ? "UNVERIFIED" : rs.getString("identity_status"),
                            responseVideoStatus,
                            sellerApprovedVideo,
    ...
    private String securityScore(Long userId) {
        Integer count = jdbcTemplate.queryForObject(
                "SELECT COUNT(*) FROM user_login_record WHERE user_id = ?",
                Integer.class,
                userId
        );
        return count != null && count > 0 ? "已记录" : "--";
    }

    private List<AccountSecurityResponse.LoginDeviceResponse> loadRecentLoginDevices(Long userId) {
        return jdbcTemplate.query("""
                SELECT device_name, login_at, ip_location
                FROM user_login_record
                WHERE user_id = ?
                ORDER BY login_at DESC, id DESC
                LIMIT 5
                """, (rs, rowNum) -> new AccountSecurityResponse.LoginDeviceResponse(
                rs.getString("device_name"),
                rs.getTimestamp("login_at").toLocalDateTime().toString(),
                rs.getString("ip_location"),
                "平台记录"
        ), userId);
    }

    private String latestIpLocation(Long userId) {
        ensureLoginRecordTable();
        List<String> rows = jdbcTemplate.query("""
                ...
                ORDER BY login_at DESC, id DESC
                LIMIT 1
                """, (rs, rowNum) -> rs.getString("ip_location"), userId);
        return rows.isEmpty() ? "IP属地未知" : rows.get(0);
    }
