  ...
  <view class="page-shell report-page">
    ...
    <view class="reason-card ds-card">
      ...
      <view class="upload-box tapable" @click="chooseEvidence">
        ...
        <view>
          <view class="upload-title">上传票据 {{ evidence.length }}/6</view>
          <view class="upload-desc">生成服务端上传票据后，提交举报时才会随表单提交。</view>
        </view>
      </view>
...
async function submit() {
  ...
  try {
    ...
    uni.showModal({ title: '已提交', content: '举报已由服务端接收；审核状态、通知和后续处理以服务端审核记录为准。', showCancel: false, success: () => uni.navigateTo({ url: '/pages/notification/index' }) })
  } catch {
