<template>
  <view class="page-shell report-page">
    ...
    <view class="reason-card ds-card">
      ...
      <view class="upload-box tapable" @click="chooseEvidence">
        ...
        <view>
          ...
          <view class="upload-desc">已生成 {{ evidence.length }} 张上传票据，上限 6 张</view>
        </view>
      </view>
    ...
    <view class="safe-card ds-card">
      ...
      <view class="safe-line">举报处理以平台审核记录为准，上传票据不代表业务已受理。</view>
  ...
  uni.chooseImage({
    ...
    success: async (res) => {
      ...
      try {
        ...
        uni.showToast({ title: '上传票据生成失败', icon: 'none' })
      } finally {
        uni.hideLoading()
      }
    }
...
async function submit() {
  ...
  try {
    ...
    uni.showModal({ title: '已提交', content: '举报已由平台接收；审核状态、通知和后续处理以平台审核记录为准。', showCancel: false, success: () => uni.navigateTo({ url: '/pages/notification/index' }) })
  } catch {
