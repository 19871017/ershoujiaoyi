  ...
  <view class="page-shell logistics-page">
    <view class="hero ds-card">
      <view>
        ...
        <view class="page-desc">查看卖家发货、同城交付和订单履约进度。</view>
      </view>
...
const errorText = ref('')
const shippingTypeText = computed(() => order.value?.shippingType === 'MEETUP' ? '同城当面交付' : '快递邮寄')
const timeline = computed(() => {
  const item = order.value
  if (!item) return []
  return [
    { title: item.shippedAt ? '卖家已发货' : '等待卖家发货', desc: item.shippedAt ? '卖家已提交履约信息，请留意物流或同城交付约定。' : '付款完成后卖家需要提交真实发货信息。', time: item.shippedAt || '' },
    { title: item.paidAt ? '订单已付款' : '等待付款', desc: item.paidAt ? '订单付款状态来自后端订单详情，履约与结算请以订单状态为准。' : '买家完成付款后才会进入后续履约流程。', time: item.paidAt || '' },
    { title: '订单已创建', desc: '订单已进入小原圈交易流程。', time: item.createdAt }
  ]
})
