...
const auditNo = ref('')
function isValidAuditNo(value: string) { return /^AU-[A-Z0-9][A-Z0-9-]{3,63}$/i.test(value.trim()) }
function readQuery(){const pages=getCurrentPages(); const current=pages.length?pages[pages.length-1] as unknown as {options?:Record<string,string>}:undefined; const rawAuditNo = current?.options?.auditNo || ''; auditNo.value = isValidAuditNo(rawAuditNo) ? rawAuditNo : ''}
function goAuditDetail(){if (!isValidAuditNo(auditNo.value)) return uni.showToast({title:'缺少真实审核单号',icon:'none'}); uni.redirectTo({url:`/pages/admin/audit/detail/index?auditNo=${encodeURIComponent(auditNo.value)}`})}
onMounted(readQuery)
</script>
