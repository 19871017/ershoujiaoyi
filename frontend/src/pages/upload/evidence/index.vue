<template>
  <view class="page-shell upload-page">
    ...
    <view class="image-grid">
      ...
      <view v-if="images.length < 9" class="image-box add tapable" @click="chooseMedia">＋</view>
    </view>
...
function chooseVideoIdentity(){
  if (images.value.length >= 1) return uni.showToast({ title:'视频认证上传票据每次仅保留 1 条', icon:'none' })
  uni.chooseVideo({ sourceType:['camera','album'], compressed:true, maxDuration:60, async success(res){
    try {
      const path = res.tempFilePath
      if (!path || path.startsWith('local://') || path.includes('placeholder')) throw new Error('视频文件无效，请重新选择')
      ...
      images.value = [ticket.storageUrl]
      uni.showToast({title:'已生成上传票据 1 条',icon:'none'})
    } catch (error) {
      uni.showToast({title:error instanceof Error ? error.message : '视频上传票据创建失败',icon:'none'})
    }
  }, fail(){ uni.showToast({title:'未选择视频文件',icon:'none'}) } })
}
