...
async function loadDetail(){
  loadError.value = ''
  if (!backendProductId.value) { loadError.value = '缺少有效商品编号'; return }
  ...
  catch { form.title=''; form.description=''; form.price=''; images.value=[]; loadError.value = '商品详情加载失败，请稍后重试' }
}
...
function toggleTradePreference(_field: 'serverTradeOnly' | 'serverChatRecord') { uni.showToast({ title:'交易展示项暂不可变更，请提交商品修改后以平台审核结果为准', icon:'none' }) }
