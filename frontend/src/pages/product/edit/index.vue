...
function toggleTradePreference(_field: 'serverTradeOnly' | 'serverChatRecord') { uni.showToast({ title:'交易展示项暂未接入服务端状态，未执行任何变更', icon:'none' }) }
...
function validate(){ if(!form.title || !form.price) return '请补全标题和价格'; if(Number(form.price)<=0) return '价格需大于0'; if(images.value.some(url => url.startsWith('local://') || url.includes('placeholder') || !url.startsWith('/uploads/product-image/'))) return '图片需先完成服务端上传票据校验'; return '' }
async function save(){
