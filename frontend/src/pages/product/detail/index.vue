...
const sellerTrustText = computed(() => '商品卖家信息以平台返回为准 · 暂无平台信用/成交统计')
...
async function toggleFavorite() {
  if (!detail.value?.productId || detail.value.productId <= 0) {
    uni.showToast({ title: '商品缺少平台 productId，未执行收藏变更', icon: 'none' })
    return
  }
  ...
  try {
    if (wasFavorited) {
      ...
      uni.showToast({ title: '取消收藏已提交平台', icon: 'none' })
    } else {
      await favoriteProduct(detail.value.productId)
      favorited.value = true
      uni.showToast({ title: '收藏已提交平台', icon: 'none' })
    }
  } catch {
    uni.showToast({ title: '收藏接口调用失败，未执行收藏变更', icon: 'none' })
  } finally {
