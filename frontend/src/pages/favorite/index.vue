  ...
  <view class="page-shell favorite-page">
    <view class="hero ds-card">
      <view>
        ...
        <view class="page-desc">仅展示后端收藏接口返回的商品，加载失败不展示本地收藏样例。</view>
      </view>
    ...
    <view v-else-if="filtered.length === 0" class="empty-card ds-card">暂无后端收藏商品，未展示本地收藏样例</view>
    ...
    <view class="fav-grid">
      <view v-for="item in filtered" :key="item.productId" class="fav-card ds-card tapable" @click="openProduct(item.productId)">
        ...
        <view class="meta">后端商品 · {{ statusLabel(item.status) }}</view>
...
function openProduct(productId: number) {
  if (!productId || productId <= 0) { uni.showToast({ title: '收藏商品缺少后端 productId，未打开本地样例', icon: 'none' }); return }
  uni.navigateTo({ url: `/pages/product/detail/index?productId=${productId}` })
}
async function unfav(productId: number) {
  if (!productId || productId <= 0) { uni.showToast({ title: '收藏商品缺少后端 productId，未执行任何变更', icon: 'none' }); return }
  try {
...
async function loadFavorites() {
  ...
  try {
    ...
    favorites.value = []
    loadMessage.value = `收藏列表接口加载失败，未展示本地收藏样例：${error instanceof Error ? error.message : '请稍后重试'}`
  } finally {
    loading.value = false
  }
}
