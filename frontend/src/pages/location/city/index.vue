  ...
  <view class="page-shell city-page">
    <view class="hero ds-card">
      <view>
        ...
        <view class="page-desc">城市偏好会写入服务端个人资料；正式位置状态以平台或系统定位记录为准。</view>
      </view>
    ...
    <view class="safe-card ds-card">
      ...
      <view class="desc">城市偏好写入服务端资料后可用于本次页面筛选；订单、支付、发货和售后状态以平台记录为准。</view>
    </view>
...
async function saveCity() {
  ...
  saving.value = true
  try {
    const updated = await updateMyProfile({ nickname: profile.nickname, mainRole: profile.mainRole, gender: profile.gender, city: selected.value, bio: profile.bio })
