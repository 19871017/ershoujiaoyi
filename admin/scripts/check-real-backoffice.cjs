...
const requiredFiles = [
  ...
  'src/pages/audit/audit-review-policy.ts',
  'src/pages/chat-trace/index.vue',
  'src/pages/chat-trace/chat-trace-links.ts',
  'src/pages/community-trace/index.vue',
  'src/pages/community-trace/community-trace-links.ts',
...
const forbidden = [
  ...
  { file: 'src/pages/products/index.vue', pattern: /预览商品|本地商品样例|假成功|伪成功|mock|demo/i, message: 'product page must use backend product records and avoid local samples or fake success' },
  ...
  { file: 'src/pages/audit/report-handling.ts', pattern: /没有独立私聊追溯页/, message: 'report handling guide must not claim chat reports lack a real trace page' }
]
const requiredPatterns = [
  ...
  { file: 'src/pages/audit/index.vue', pattern: /auditTypeFilter/, message: 'audit workbench must expose type filters so reports can be handled as an operations queue' },
  { file: 'src/pages/audit/index.vue', pattern: /showReportsOnly/, message: 'audit workbench must keep a one-click report queue filter' },
  { file: 'src/api/modules/admin.ts', pattern: /getAdminAuditList\(query: AdminAuditListQuery/, message: 'admin audit API wrapper must send validated audit list filters to backend' },
  { file: 'src/api/modules/admin.ts', pattern: /isValidAdminAuditKeyword/, message: 'admin audit API wrapper must fail closed on invalid audit keywords before fetch' },
  ...
  { file: 'src/pages/audit/index.vue', pattern: /if \(isProductAudit\) \{[\s\S]*await approveAdminProduct\(item\.targetId \|\| ''\)[\s\S]*await load\(\)[\s\S]*return[\s\S]*\}[\s\S]*const updated = action === 'approve' \? await approveAdminAudit/, message: 'product audit approval must return after backend product approval to avoid duplicate generic audit submission' },
  { file: 'src/pages/audit/audit-review-policy.ts', pattern: /requiresVideoEvidenceBeforeApproval/, message: 'audit review policy must centralize video identity evidence-before-approval rules' },
  { file: 'src/pages/audit/index.vue', pattern: /requiresVideoEvidenceBeforeApproval/, message: 'audit list must block pending video identity direct approval and route operators to detail review' },
  { file: 'src/pages/audit/detail.vue', pattern: /loadVideoEvidence/, message: 'audit detail must force authorized evidence loading before video identity approval' },
  { file: 'src/pages/audit/detail.vue', pattern: /canApproveAuditFromDetail/, message: 'audit detail approval button must depend on video identity evidence review policy' },
  { file: 'src/pages/audit/detail.vue', pattern: /approveAdminAudit/, message: 'audit detail must submit approved review through backend audit endpoint after evidence checks' },
  { file: 'src/pages/audit/detail.vue', pattern: /rejectAdminAudit/, message: 'audit detail must keep rejection available from the detail review flow' },
  { file: 'src/pages/audit/index.vue', pattern: /afterSalesAuditTraceLocation/, message: 'audit workbench must expose after-sales trace actions for traceable targets' },
  ...
  { file: 'src/pages/audit/detail.vue', pattern: /reportHandlingGuide/, message: 'audit detail must expose report handling guidance for operator review decisions' },
  { file: 'src/pages/audit/index.vue', pattern: /maskSensitiveMediaText/, message: 'audit list must mask raw sensitive media paths in review summaries' },
  { file: 'src/pages/audit/detail.vue', pattern: /maskSensitiveMediaText/, message: 'audit detail must mask raw sensitive media paths in review summaries' },
  { file: 'src/pages/chat-trace/index.vue', pattern: /safeMessageContent/, message: 'chat trace fallback rendering must mask raw sensitive media paths' },
  { file: 'src/pages/after-sales/index.vue', pattern: /safeDetailDescription/, message: 'after-sales description rendering must mask raw sensitive media paths' },
  { file: 'src/pages/audit/index.vue', pattern: /chatAuditTraceLocation/, message: 'audit workbench must expose chat trace actions for traceable chat reports' },
  { file: 'src/pages/audit/detail.vue', pattern: /chatAuditTraceLocation/, message: 'audit detail must expose chat trace actions for traceable chat reports' },
  ...
  { file: 'src/pages/audit/report-handling.ts', pattern: /私聊追溯/, message: 'report handling guide must direct chat reports to the real chat trace page' },
  { file: 'src/router/index.ts', pattern: /path: 'chat-trace'/, message: 'router must expose chat trace list route' },
  { file: 'src/router/index.ts', pattern: /path: 'chat-trace\/:conversationId'/, message: 'router must expose chat trace detail route' },
  { file: 'src/router/index.ts', pattern: /path: 'community-trace'/, message: 'router must expose community trace list route' },
  { file: 'src/router/index.ts', pattern: /path: 'community-trace\/:postId'/, message: 'router must expose community trace detail route' },
  { file: 'src/router/index.ts', pattern: /path: 'products'/, message: 'router must expose product management list route' },
  { file: 'src/router/index.ts', pattern: /path: 'products\/:productId'/, message: 'router must expose product management detail route' },
  { file: 'src/store/modules/auth.ts', pattern: /pattern: \/\^\\\/chat-trace\(\?:\\\/\|\$\)\//, message: 'chat trace routes must require explicit audit:read permission' },
  { file: 'src/store/modules/auth.ts', pattern: /pattern: \/\^\\\/community-trace\(\?:\\\/\|\$\)\//, message: 'community trace routes must require explicit audit:read permission' },
  { file: 'src/store/modules/auth.ts', pattern: /pattern: \/\^\\\/products\(\?:\\\/\|\$\)\//, message: 'product management routes must require explicit audit:read permission' },
  { file: 'src/store/modules/auth.ts', pattern: /path: '\/chat-trace'[\s\S]*permission: 'audit:read'/, message: 'dashboard quick actions must expose chat trace only to audit:read sessions' },
  { file: 'src/store/modules/auth.ts', pattern: /path: '\/community-trace'[\s\S]*permission: 'audit:read'/, message: 'dashboard quick actions must expose community trace only to audit:read sessions' },
  { file: 'src/store/modules/auth.ts', pattern: /path: '\/products'[\s\S]*permission: 'audit:read'/, message: 'dashboard quick actions must expose product management only to audit:read sessions' },
  { file: 'src/api/modules/admin.ts', pattern: /getAdminChatConversations/, message: 'admin API wrapper must expose chat conversation trace list endpoint' },
  { file: 'src/api/modules/admin.ts', pattern: /getAdminChatConversationMessages/, message: 'admin API wrapper must expose chat conversation message trace endpoint' },
  { file: 'src/api/modules/admin.ts', pattern: /beforeSeq/, message: 'admin chat trace API wrapper must support beforeSeq pagination for long conversations' },
  { file: 'src/api/modules/admin.ts', pattern: /isValidAdminChatTraceKeyword/, message: 'admin API wrapper must fail closed on invalid chat trace keywords before fetch' },
  { file: 'src/pages/chat-trace/chat-trace-links.ts', pattern: /chatAuditTraceLocation/, message: 'chat trace link helper must map chat audit targets to the trace page' },
  { file: 'src/pages/chat-trace/index.vue', pattern: /getAdminChatConversations/, message: 'chat trace page must load backend chat conversations' },
  { file: 'src/pages/chat-trace/index.vue', pattern: /getAdminChatConversationMessages/, message: 'chat trace page must load backend chat messages' },
  { file: 'src/pages/chat-trace/index.vue', pattern: /loadOlderMessages/, message: 'chat trace page must allow loading earlier messages instead of only latest messages' },
  { file: 'src/pages/chat-trace/chat-trace-media.ts', pattern: /contentJson\.trim\(\)\.replace/, message: 'chat trace helper must parse legacy escaped message contentJson' },
  { file: 'src/pages/chat-trace/chat-trace-media.ts', pattern: /parseChatTraceMessage\(\{ \.\.\.message, contentJson: value \}\)/, message: 'chat trace helper must parse nested double-encoded message contentJson' },
  { file: 'src/pages/chat-trace/chat-trace-media.ts', pattern: /\/uploads\/chat-voice\//, message: 'chat trace helper must allow only backend chat voice uploads for playback' },
  { file: 'src/pages/chat-trace/chat-trace-media.ts', pattern: /\/uploads\/chat-image\//, message: 'chat trace helper must allow only backend chat image uploads for trace links' },
  { file: 'src/pages/chat-trace/chat-trace-media.ts', pattern: /isSafeTraceUploadUrl/, message: 'chat trace helper must whitelist chat media URLs before rendering links or players' },
  { file: 'src/pages/chat-trace/index.vue', pattern: /<audio[\s\S]*controls/, message: 'chat trace page must render playable audio controls for valid voice messages' },
  { file: 'src/api/modules/admin.ts', pattern: /getAdminCommunityPosts/, message: 'admin API wrapper must expose community trace list endpoint' },
  { file: 'src/api/modules/admin.ts', pattern: /getAdminCommunityPostDetail/, message: 'admin API wrapper must expose community trace detail endpoint' },
  { file: 'src/api/modules/admin.ts', pattern: /isValidAdminCommunityTraceKeyword/, message: 'admin API wrapper must fail closed on invalid community trace keywords before fetch' },
  { file: 'src/api/modules/admin.ts', pattern: /getAdminProductList/, message: 'admin API wrapper must expose product management list endpoint' },
  { file: 'src/api/modules/admin.ts', pattern: /getAdminProductDetail/, message: 'admin API wrapper must expose product management detail endpoint' },
  { file: 'src/api/modules/admin.ts', pattern: /isValidAdminProductKeyword/, message: 'admin product API wrapper must fail closed on invalid product keywords before fetch' },
  { file: 'src/pages/products/index.vue', pattern: /getAdminProductList/, message: 'product management page must load backend admin product list' },
  { file: 'src/pages/products/index.vue', pattern: /getAdminProductDetail/, message: 'product management page must load backend admin product detail' },
  { file: 'src/pages/products/index.vue', pattern: /approveAdminProduct/, message: 'product management page must approve through backend product endpoint' },
  { file: 'src/pages/products/index.vue', pattern: /rejectAdminProduct/, message: 'product management page must reject through backend product endpoint' },
  { file: 'src/pages/products/index.vue', pattern: /canReviewAudit/, message: 'product management review actions must require explicit audit review permission' },
  { file: 'src/pages/products/index.vue', pattern: /isValidAdminProductKeyword/, message: 'product management filters must validate keywords before backend fetch' },
  { file: 'src/pages/products/index.vue', pattern: /\/api\/admin\/products\*/, message: 'product management page must state that product operations use real admin product APIs' },
  { file: 'src/pages/community-trace/community-trace-links.ts', pattern: /communityAuditTraceLocation/, message: 'community trace link helper must map community audit targets to the trace page' },
  { file: 'src/pages/community-trace/index.vue', pattern: /getAdminCommunityPosts/, message: 'community trace page must load backend community posts' },
  { file: 'src/pages/community-trace/index.vue', pattern: /getAdminCommunityPostDetail/, message: 'community trace page must load backend community post detail' },
  { file: 'src/pages/community-trace/community-trace-media.ts', pattern: /community-image[\s\S]*isSafeCommunityImageUrl/, message: 'community trace helper must allow only backend community image uploads for evidence links' },
  { file: 'src/pages/audit/report-handling.ts', pattern: /社区追溯/, message: 'report handling guide must direct community reports to the real community trace page' },
  ...
  { file: 'src/api/modules/admin.ts', pattern: /AdminWithdrawalReviewDetail/, message: 'admin withdrawal detail API must expose review context wrapper' },
  { file: 'src/api/modules/admin.ts', pattern: /WD-\(\?:\\d\{8\}-\\d\{4,\}\|\\d\{12,17\}-\\d\{1,6\}\)/, message: 'admin withdrawal number guard must accept backend millisecond withdrawal numbers as well as legacy date numbers' },
  { file: 'src/pages/finance/withdrawals/index.vue', pattern: /用户与实名/, message: 'withdrawal detail page must show user identity context for finance review' },
  { file: 'src/pages/finance/withdrawals/index.vue', pattern: /钱包余额/, message: 'withdrawal detail page must show wallet balance context for finance review' },
  { file: 'src/pages/finance/withdrawals/index.vue', pattern: /最近钱包流水/, message: 'withdrawal detail page must show recent wallet ledgers for finance review' },
  ...
  { file: 'src/pages/after-sales/index.vue', pattern: /afterSalesEvidenceMediaUrl/, message: 'after-sales page must load evidence through the authorized backend media endpoint' },
  { file: 'src/pages/after-sales/index.vue', pattern: /requestBlob\(\{ url: afterSalesEvidenceMediaUrl/, message: 'after-sales evidence must be fetched as an authorized blob instead of opened as raw uploads' },
  { file: 'src/pages/after-sales/index.vue', pattern: /URL\.createObjectURL/, message: 'after-sales evidence viewing must use object URLs returned from authorized blob responses' },
  { file: 'src/pages/after-sales/index.vue', pattern: /URL\.revokeObjectURL/, message: 'after-sales evidence object URLs must be revoked when detail changes or component unmounts' },
  ...
  { file: 'src/api/modules/admin.ts', pattern: /opsSummary/, message: 'admin user detail API must expose operations summary for backoffice review' },
  { file: 'src/pages/users/index.vue', pattern: /运营复盘/, message: 'user page must show operations summary for backoffice review' },
  { file: 'src/pages/users/index.vue', pattern: /openUserChatTrace/, message: 'user page must expose chat trace action for user review' },
  { file: 'src/pages/users/index.vue', pattern: /openUserWithdrawalTrace/, message: 'user page must expose withdrawal trace action for user review' },
