...
>
> 当前实现说明：小原圈 MVP 已落地 HTTP 同步/5 秒轮询补拉，而不是完整 WebSocket 长连接；`ws-gateway` 仍是后续实时化边界。当前消息类型已支持 `TEXT`、`IMAGE`、`VOICE`，并已支持 2 分钟内撤回、个人清空聊天记录、后台私聊追溯与授权媒体播放。
...
- 不做页面层直连 socket 管理。
- 不把图片二进制、语音二进制、视频二进制、文件二进制塞进消息体。
...
```json
{
  "url": "/uploads/chat-image/1001/abc.jpg",
  "mimeType": "image/jpeg",
  "width": 1080,
  "height": 1440,
  "sizeBytes": 345678
...
- `url` 必须是后端媒体上传票据签发并完成上传后的 `/uploads/chat-image/` 存储地址。
- `mime_type` 必须是图片类型。
...
```json
{
  "url": "/uploads/chat-voice/1001/voice.webm",
  "durationMs": 2400,
  "sizeBytes": 18888,
  "mimeType": "audio/webm"
}
...
- 语音上传必须走 `CHAT_VOICE` 媒体上传票据。
- `url` 必须是 `/uploads/chat-voice/` 存储地址。
- `durationMs` 必须大于 0，并受服务端上限约束。
- `sizeBytes` 不得超过服务端语音上传上限。
- 前端只引用授权后的媒体地址播放，不得把音频二进制写入消息体。
...
- 只允许发送者撤回自己的消息。
- 当前窗口为发送后 2 分钟内。
- 撤回后用户端展示“消息已撤回”或“语音已撤回”。
- 撤回媒体不得继续对普通用户授权播放。
- 后台在具备 `chat:trace` 权限且写入审计后，可以追溯原始消息与授权媒体。

清空：

- 清空是当前账号视角的聊天记录清理，不删除对方视角记录。
- 清空后当前账号不得继续读取清空范围内的普通聊天媒体。
- 清空不等于删除平台审计留痕。
...
- 登录成功后。
- 当前 HTTP 轮询定时触发，或未来 WebSocket / 长连接重连后。
- 进入会话详情页后。
- 前台恢复、网络恢复后。
...
- 页面只订阅状态，不直接控制底层连接细节。
- 当前 H5 会话页采用接口同步与轮询补拉，这属于 MVP 过渡实现；后续接入 WebSocket 时仍必须保持页面不直接管理底层连接。
